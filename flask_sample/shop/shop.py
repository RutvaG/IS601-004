...
def shop_list():
    rows = []
    args = []
    #UCID: rg695 Date: 05/05/23
    name = request.args.get("name")
    category = request.args.get("category")
    price = request.args.get("price")
    query = "SELECT id, name, description, stock, unit_price FROM IS601_S_Products WHERE stock > 0 "
    if name:
...
def purchase():
    ...
    order = {}
    try:
        DB.getDB().autocommit = False # making transaction
