...
def add_task(name: str, description: str, due: str):
    ...
    try:
        due_data = str_to_datetime(due)
    except:
        print('Addition rejected due to invalid due date string')
        return
    task.update({'name': name, 'description': description, 'due': due_data, 'lastActivity': datetime.now()})
    tasks.append(task)
...
def update_task(index: int, name: str, description:str, due: str):
    ...
    if description and task.get('description') != description:
        ...
        updated = True
    if due and task.get('due') != due:
        task.update({'due': str_to_datetime(due)})
        updated = True
    task.update({'lastActivity': datetime.now()})
    if updated:
...
def get_incomplete_tasks():
    ...
    # include your ucid and date as a comment of when you implemented this, briefly summarize the solution
    _tasks = [task for task in tasks if not task.get('done')]
    list_tasks(_tasks)
...
def get_overdue_tasks():
    ...
    # pass that list into list_tasks()
    # include your ucid and date as a comment of when you implemented this, briefly summarize the solution
    _tasks = [task for task in tasks if str_to_datetime(task.get('due')) < datetime.now() and not task.get('done')]
    list_tasks(_tasks)
...
def get_time_remaining(index):
    ...
    if index < 0 or index > len(tasks) - 1:
        ...
        return
    task = tasks[index]
    if str_to_datetime(task.get('due')) < datetime.now():
        time_diff = datetime.now() - str_to_datetime(task.get('due'))
        print('Your task is overdue by {} days, {} hours, {} minutes, {} seconds'.format(
            time_diff.days, time_diff.seconds//3600, (time_diff.seconds//60) % 60, time_diff.seconds % 60))
    else:
        time_diff = str_to_datetime(task.get('due')) - datetime.now()
        print('Your task is due in {} days, {} hours, {} minutes, {} seconds'.format(
            time_diff.days, time_diff.seconds // 3600, (time_diff.seconds // 60) % 60, time_diff.seconds % 60))
