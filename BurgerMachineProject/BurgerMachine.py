    ...
    def run(self):
        try:
            if self.currently_selecting == STAGE.Bun:
                ...
                patty = input(f"Which type of patty would you like {', '.join(list(map(lambda f:f.name.lower(), filter(lambda f: f.in_stock(), self.patties))))}? Or type next.\n")
        ...
        # handle OutOfStockException
            ...
            print(f'Item ordered is out of stock in {self.currently_selecting.name} category. Kindly pick another item.')
        ...
        # handle NeedsCleaningException
            ...
            print('The Burger Machine cannot take any more orders')
            # prompt user to type "clean" to trigger clean_machine()
            clean = input("What would you like to do? (clean or quit)\n")
            if clean == "clean":
            # print a message whether or not the machine was cleaned
                self.clean_machine()
                print("Machine cleaned and ready for use!")
            else:
                exit()
            # any other input is ignored
        # handle InvalidChoiceException
        except InvalidChoiceException:
            print(f'This is an invalid choice in {self.currently_selecting.name} category. Please choose from the options listed above')
        ...
        # handle ExceededRemainingChoicesException
            ...
            print(f'You have exceeded the maximum allowed choices for the {self.currently_selecting.name} category.')
            if self.currently_selecting == STAGE.Patty:
                self.currently_selecting = STAGE.Toppings 
            elif self .currently_selecting == STAGE.Toppings:
                self.currently_selecting = STAGE.Pay 
        ...
        # handle InvalidPaymentException
                ...
                print("Invalid payment amount. Input right amount (format x.x).")
